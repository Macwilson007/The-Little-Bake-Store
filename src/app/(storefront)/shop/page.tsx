...
function ShopContent() {
    ...
    const filteredProducts = useMemo(() => {
        ...
        if (selectedCategory || categoryParam) {
            ...
            result = result.filter((p) => p.category?.slug === cat || p.category === cat);
        }
        ...
        if (selectedFlavors.length > 0) {
            result = result.filter((p) => selectedFlavors.includes(p.flavor));
        }

        // Occasion filter
        if (selectedOccasions.length > 0) {
            result = result.filter((p) => p.occasions?.some((o: string) => selectedOccasions.includes(o)));
        }

        // Diet filter
        if (selectedDiet.length > 0) {
            result = result.filter((p) => p.dietTypes?.some((d: string) => selectedDiet.includes(d)));
        }
    ...
    const activeCategoryData = dbCategories.find((c) => c.slug === activeCategory) || categories.find(c => c.slug === activeCategory);
    const activeSubcategoryData = activeCategoryData?.subcategories?.find((s: any) => s.slug === subcategoryParam);
